...
key_byte = bytestobits(key)
key_str = "00"*10
##keystream 00000000000000000005 16
##keystream FFFFFFFFFFFFFFFFFFFF 16
print("Commands:\n    keystream [iv] [number of bytes]\n    guess [key]\n")
for i in range(5000):
	#l = input().strip().split()#recvline(48).strip().split() ##recvline(48).strip().split()
	l = "keystream "+ key_str +" 16"
	l = l.strip().split()
	print(l)
	if l[0] == "keystream":
		...
		if 1 <= numbytes <= 16 and len(iv) == 10:
			...
			key_str = binascii.hexlify(stream)
			key_str = str(key_str)[2:-13]
			out_byte = bytestobits(binascii.hexlify(stream))
			if key_byte in out_byte:
				print("Con !!")
				print(key)
				print(key_byte)
				print(out_byte)
